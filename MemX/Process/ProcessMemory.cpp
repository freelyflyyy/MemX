...
namespace MemX {
	...
	NTSTATUS ProcessMemory::Read(PTR_T baseAddr, PVOID pResult, size_t dwSize, bool skipUncommited) {
		DWORD64 dwRead = 0;
		if ( !baseAddr ) {
	...
	NTSTATUS ProcessMemory::Read(const std::vector<PTR_T>& addrList, PVOID pResult, size_t dwSize, bool skipUncommited) {
		if ( addrList.empty() ) {
	...
	NTSTATUS ProcessMemory::Write(PTR_T baseAddr, LPCVOID pData, size_t dwSize) {
		if ( !baseAddr )
			return STATUS_INVALID_ADDRESS;
	...
	NTSTATUS ProcessMemory::Write(const std::vector<PTR_T>& addrList, LPCVOID pData, size_t dwSize) {
		if ( addrList.empty() ) {
			return STATUS_INVALID_PARAMETER;
		}
